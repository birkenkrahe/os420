...
* NEXT Networking - w14s25 (21-Apr)

  * [ ] Last two *DataCamp* assignments for EXTRA CREDIT only
  * [ ] *Quiz 7-9* are live (bash scripting/permissions/processes)
  * [ ] Play the quizzes by April 22 (Friday) 4 pm
  * [ ] *Test 3* will be online - open Thu-28-Apr to Fri-28-Apr
  * [ ] *Test 3* will not have any new questions - only Q7-Q9
  * [ ] *Topic* today: Networking under Linux

** Networking

   * [ ] ~ping~ other network hosts
   * [ ] ~traceroute~ to print network travel routes
   * [ ] ~ip~ to show/manipulate network devices
   * [ ] ~netstat~ to print network stats
   * [ ] ~ftp~ for internet file transfer
   * [ ] ~wget~ for non-interactive network downloads
   * [ ] ~ssh~ for remote shell login

** Bonus features

   * [ ] How to connect your Pi to GDrive using ~rclone~
   * [ ] Emacs special: using ~Dired~ and ~shell~ remotely with ~Tramp~
    
* TODO Archiving and backup - w15s26 (26-Apr)
* TODO Compiling source code from scratch - w15s27 (28-Apr)
* TODO Review: quizzes, tests, final exam preparation (3-May)
* References
