...
** Shell warm-up exercise
   ...
   Complete the exercise on the command line (10 min):
   ...
   * verify that you're there
   * move ~sample.txt~ to your current directory
   * make 2 copies of ~sample.txt~
   * name them ~sample_1~ and ~sample_2~
   * view both files using ~less~
   * leave ~less~
